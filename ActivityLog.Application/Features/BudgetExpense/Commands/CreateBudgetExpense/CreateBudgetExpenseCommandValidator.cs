...
{
	public class CreateBudgetExpenseCommandValidator : AbstractValidator<CreateBudgetExpenseCommand>
	{
		public CreateBudgetExpenseCommandValidator()
		{
			RuleFor(p => p.BudgetId)
				.NotEmpty().WithMessage("{BudgetId} no puede estar vacio")
				.NotNull().WithMessage("{BudgetId} no puede ser null")
				.GreaterThan(0).WithMessage("{BudgetId} El valor debe ser mayor que cero.");
